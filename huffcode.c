/* Huffman Tree
 * 
 * This is a little code demonstrating the Huffman algorithm.
 */
...
void DataIf(struct RawData* rawData, int aux) {
    ...
    if (left < rawData->size
        && rawData->array[left]->freq
               < rawData->array[lower]->freq)
        lower = left;
 
    if (right < rawData->size
        && rawData->array[right]->freq
               < rawData->array[lower]->freq)
        lower = right;
 
    if (lower != aux) {
        swapElements(&rawData->array[lower],
                        &rawData->array[aux]);
